...
face_cascade = cv.CascadeClassifier('haarcascade_frontalface_default.xml') # We load the cascade for the face.
face_vetor = []
...
while True: 
    ...
    cv.imshow('FACIAL EXPRESSION RECOGNITION VIDEO STREAM', canvas) 
...
cv.destroyAllWindows() # We destroy all the windows inside which the images were displayed.